...
var SettingStore = require('../stores/SettingStore').default
...
var CommentMixin = {
        ...
        ItemStore.fetchCommentAncestors(comment, result => {
            if (process.env.NODE_ENV !== 'production') {
                console.info(
                    'fetchAncestors(' +
                    comment.id +
                    ') took' + result.timeTaken + 'ms For' +
                    result.itemCount + ' item' + pluralise(result.cacheHits) + '(' + (result.cacheHits / result.itemCount * 100).toFixed(1) + '%)'
                )
            }

            if (!this.isMounted) {
                if (process.env.NODE_ENV !== 'production')
                    console.info("... but the commit isn't mounted")
                return
            }
    ...
    renderCommentMeta(comment, options) {
        if (comment.dead && !SettingStore.showDead) {
    ...
    renderCommentText(comment, options) {
        return <div className="Comment__text">
            {(!comment.dead || SettingStore.showDead) ? <div dangerouslySetInnerHTML={{ __html: comment.text }} /> : '[dead]'}
            {SettingStore.replyLinks && options.replyLinks && !comment.dead && <p>
